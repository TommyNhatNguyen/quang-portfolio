...
const WorkPage = () => {
  ...
  useEffect(() => {
    ...
    return loaderEvents.onDone(() => setAnimationReady(true));
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  useGSAP(
    () => {
      if (!animationReady || !workPage) return;

      const typingSequence = workPage.introduction.map((item, idx, arr) => ({
        text: item.text,
        pauseAfter: idx === arr.length - 1 ? 0 : 0.5,
        deleteCount: (idx === arr.length - 1 ? 0 : "all") as number | "all",
      }));
      ...
      function restoreFinalState() {
        ...
        if (!hasTextNode) {
          titleEl.insertBefore(
            document.createTextNode(
              typingSequence[typingSequence.length - 1].text,
            ),
            cursorEl,
          );
        }
        ...
        document
          ...
          .forEach((itemEl, i) => {
            ...
            const descSep = itemEl.querySelector(".description__separator");
            const descLast = itemEl.querySelector(
              ".description__text:last-child",
            );
            if (title) title.textContent = workItem.title;
            if (descFirst) descFirst.textContent = workItem.description;
            if (descSep) descSep.textContent = " | ";
            if (descLast) descLast.textContent = workItem.year;
          });
      ...
      function setupScroll() {
        ...
        if (firstItem && lastItem) {
          ...
          const visibleHeight =
            footerTop - firstItem.getBoundingClientRect().top;
    ...
    { dependencies: [animationReady, workPage] },
  );
  ...
  return (
    <div className="work-container">
      ...
      <div className="work-list">
        {workPage?.work_items.map((item, index) => {
